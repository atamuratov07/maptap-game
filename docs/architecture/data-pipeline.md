...
graph TD
    ...
    Backfill["07_backfill_uz_latn_catalog.mjs<br/>(npm run backfill:country-uz-latn)"]
    ...
    CatalogJson --> Backfill
    Backfill --> CatalogJson
```
...
| `npm run backfill:country-uz-latn` | `@maptap/country-build`'s `backfill:uz-latn` script (`node scripts/07_backfill_uz_latn_catalog.mjs`) | Uzbek Latin-script names added to the generated catalog |

See [`source-tree.md`](source-tree.md) for the full script list inside `country-build`.
